...
function extractListingData(html) {
  ...
  Array.from(items).forEach((item, index) => {
    ...
    try {
      ...
      // RESTORED: Get clean sold price using WORKING selectors + CURRENCY DETECTION
      ...
      // Add currency code detection and formatting
      if (soldPrice) {
        // Detect currency and add appropriate code
  ...
  try {
    ...
    res.status(500).json({ 
      ...
      error: error.message 
          ...
          // Default to USD for dollar sign (most eBay sales)
  ...
  try {
    ...
    res.status(500).json({ 
      ...
      error: error.message 
        ...
        } else if (soldPrice.includes('£')) {
          soldPrice = soldPrice + ' GBP';
        } else if (soldPrice.includes('€')) {
          soldPrice = soldPrice + ' EUR';
        } else if (soldPrice.includes('¥')) {
          soldPrice = soldPrice + ' JPY';
        } else if (soldPrice.includes('C
  ...
  try {
    ...
    res.status(500).json({ 
      ...
      error: error.message 
          ...
          soldPrice = soldPrice + ' CAD';
        } else if (soldPrice.includes('A
  ...
  try {
    ...
    res.status(500).json({ 
      ...
      error: error.message 
          ...
          soldPrice = soldPrice + ' AUD';
        } else if (soldPrice.match(/\d+\.\d{2}/)) {
          // If it's a decimal number without currency symbol, assume USD
          soldPrice = '
  ...
  try {
    ...
    res.status(500).json({ 
      ...
      error: error.message 
        ...
        // Clean up any double currency codes
        soldPrice = soldPrice.replace(/(USD|GBP|EUR|JPY|CAD|AUD)\s+(USD|GBP|EUR|JPY|CAD|AUD)/, '$1');
      }
